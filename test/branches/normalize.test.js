...
test('LTS branches - initial state', t => {
  ...
  t.deepEqual(normalize.lts({lts, release}).map(({type, name, range, channel}) => ({type, name, range, channel})), [
    {type: 'lts', name: '1.1.x', range: '>=1.1.0 <1.0.0', channel: '1.1.x'},
    {type: 'lts', name: '1.2.x', range: '>=1.2.0 <1.0.0', channel: '1.2.x'},
    {type: 'lts', name: '1.x', range: '>=1.3.0 <1.0.0', channel: '1.x'},
  ]);
});
...
test('LTS branches - cap range to first release present on default branch and not in any LTS one', t => {
  ...
  t.deepEqual(normalize.lts({lts, release}).map(({type, name, range, channel}) => ({type, name, range, channel})), [
    {type: 'lts', name: 'name', range: '>=1.1.1 <1.2.0', channel: 'name'},
    {type: 'lts', name: '1.2.x', range: '>=1.2.1 <1.3.0', channel: '1.2.x'},
    {type: 'lts', name: '1.x', range: '>=1.5.0 <1.6.0', channel: '1.x'},
    {type: 'lts', name: '2.x.x', range: '>=2.0.0 <1.6.0', channel: '2.x.x'},
  ]);
});
...
test('LTS branches - cap range to default branch last release if all release are also present on LTS branch', t => {
  ...
  t.deepEqual(normalize.lts({lts, release}).map(({type, name, range, channel}) => ({type, name, range, channel})), [
    {type: 'lts', name: '1.x', range: '>=1.3.0 <2.0.0', channel: '1.x'},
    {type: 'lts', name: '2.x.x', range: '>=2.0.0 <2.0.0', channel: '2.x.x'},
  ]);
});
